#!/usr/bin/env python3
"""
Telegram Security Bot - Main Entry Point (Production Mode)
...
class TelegramSecurityBot:
    ...
    def start_polling(self):
        ...
        while self.running:
            try:
                self.logger.info("🚀 Starting bot polling (Production mode)")
                ...
                retry_delay = min(retry_delay * 2, 300)  # exponential backoff up to 5m

    # ---------------- RUN ----------------
    def run(self):
        """Main run method (production mode)"""
        try:
            ...
            @app.route("/")
            def index():
                return render_template("base.html", status="🟢 Running (Production)")

            @app.route("/health")
            def health():
                return {"status": "ok", "running": self.running}, 200

            app.run(
                host="0.0.0.0",
                port=int(os.getenv("PORT", 5000)),
                debug=False,
                use_reloader=False,  # 🚀 important: no double Flask process
            )
